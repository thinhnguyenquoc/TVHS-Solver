...
namespace solver_TVHS_26_7
{
    public class Heuristic
    {
        ...
        {
            MyCase myCase = input.DeepCopyByExpressionTree();
            #region init array
        ...
        public MyStatictis strategy2(MyCase input, string filename)
        {
            ...
            {
                //find program which has max eff and still available
                var list1 = myCase.Programs.Where(x => x.MaxShowTime > 0).OrderBy(x => x.Duration).ToList();
                foreach (var item in list1)
                {
                    var gr = myCase.Groups.Where(x => x.Id == item.GroupId).FirstOrDefault();
                    #region group still has cota
            ...
            while (true)
            {
                ...
                var list2 = myCase.Programs.Where(x => x.MaxShowTime > 0).OrderBy(x => x.Duration).ToList();
                foreach (var pro in list2)
                {
                    var gr = myCase.Groups.Where(x => x.Id == pro.GroupId).FirstOrDefault();
                    if (gr.TotalTime >= pro.Duration)
        ...
        public MyStatictis strategy3(MyCase input, string filename)
        {
            ...
            {
                //find program which has max eff and still available
                var list1 = myCase.Programs.Where(x => x.MaxShowTime > 0).OrderBy(x => x.RevenuePerShow).ToList();
                foreach (var item in list1)
                {
                    var gr = myCase.Groups.Where(x => x.Id == item.GroupId).FirstOrDefault();
                    #region group still has cota
            ...
            while (true)
            {
                ...
                var list2 = myCase.Programs.Where(x => x.MaxShowTime > 0).OrderByDescending(x => x.Efficiency).ThenBy(x => x.Duration).ToList();
                foreach (var pro in list2)
                {
                    var gr = myCase.Groups.Where(x => x.Id == pro.GroupId).FirstOrDefault();
                    if (gr.TotalTime >= pro.Duration)
